...
| Resource limits | Request bodies are capped (default 50 MB) while streaming. Gzip is inflated under the same ceiling (bomb-safe). Concurrent validations are bounded by the Tomcat thread pool, so load cannot exhaust the heap. |
